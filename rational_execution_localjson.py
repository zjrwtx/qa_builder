...
async def process_dataset(
    ...
    local_dataset = download_and_prepare_dataset(dataset_name, cache_dir, local_dataset_dir, json_file_path)
...
def download_and_prepare_dataset(dataset_name: str, cache_dir: str, local_dataset_dir: str, json_file_path: Optional[str] = None) -> Dict[str, Dataset]:
    """Download a dataset from Hugging Face or load from a local JSON file and prepare it for local use.
    
    Args:
        dataset_name: Name of the Hugging Face dataset (ignored if json_file_path is provided)
        cache_dir: Directory to cache the downloaded dataset
        local_dataset_dir: Directory to save the processed dataset
        json_file_path: Path to a local JSON file containing the dataset (optional)
    ...
    if json_file_path:
        ...
        print(f"Loading dataset from local JSON file: {json_file_path}")
        with open(json_file_path, 'r', encoding='utf-8') as f:
            data = json.load(f)
        
        # Convert to Dataset format
        dataset = {"data": Dataset.from_list(data)}
        
        # Save to local directory for consistency
        split_path = os.path.join(local_dataset_dir, "data.json")
        with open(split_path, 'w', encoding='utf-8') as f:
            json.dump(data, f, indent=2, ensure_ascii=False)
        print(f"Saved data to {split_path}")
    else:
        ...
        if file.endswith(".json"):
            ...
            with open(os.path.join(local_dataset_dir, file), 'r', encoding='utf-8') as f:
                ...
                if isinstance(split_data, list):
                    # If the data is a list, use from_list
                    local_dataset[split_name] = Dataset.from_list(split_data)
                else:
                    # If the data is a dict, use from_dict
...
def parse_arguments():
    ...
    parser.add_argument("--json-file", type=str, default=None,
                        help="Path to a local JSON file containing the dataset (if provided, dataset name will be ignored)")
